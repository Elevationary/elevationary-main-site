...
- [ ] **[COO] Subscription_Revenue_Pipeline — Website owns P3 + P4** — CEO-ratified 2026-05-30. Website builds the Cloudflare Access policy gating `/editions/*` + `/account/*` on elevationary.com, plus the Entitlement Worker that reads the Sales-canonical R2 CRM (`sales/subscriptions/`) and serves R2 content from `newsletter/drafts/`. P4D3 path: `Operations / Fleet_Governance / Subscription_Revenue_Pipeline` (phases `P3_Cloudflare_Access_Setup` + `P4_Entitlement_Worker_and_Gated_Routes`). Depends on Sales P1 (schema must ship first). Plan + ratifications in `~/.claude/plans/i-need-to-clarify-agile-snail.md`. Locked: IdP = Google SSO + email OTP; Stripe live-state defense-in-depth in Worker (+50ms accepted); enterprise tier = OR-join on `company_id`. This **supersedes** the existing "Subscription state → Newsletter agent" item below — handoff is now via R2 CRM, not direct.
